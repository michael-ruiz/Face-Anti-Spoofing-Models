...
class MiniFASNetV2PAD(nn.Module):
    def __init__(self, num_classes=1, pretrained=True):
        ...
        self.backbone = MiniFASNetV2(num_classes=1000)
        
        if pretrained:
            # Load pretrained weights
            weights_path = 'MiniFASNetV2.pth'
    ...
    def forward(self, x):
        ...
        feat = self.backbone.fc(self.backbone.avgpool(
        ...
        out = self.backbone.classifier(feat)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler, num_epochs=10, device='cuda'):
    ...
    for epoch in range(num_epochs):
        ...
        if val_auc > best_auc:
            best_auc = val_auc
            torch.save(model.state_dict(), 'best_model_minifasnet_pad.pth')
...
if __name__ == '__main__':
    ...
    # Combine histories and plot results
    history = {
